"""
classes for paper comparing discrete and continuous-time methods

DT_IFP: discrete-time IFP (stationary and age-dependent)
CT_stat_IFP: continuous-time stationary IFP
CT_nonstat_IFP: continuous-time age-dependent IFP

All "solve" methods return V, c, time, i.

In the discrete-time setting, max_age is INFERRED from dt (which equals dA)
scipy.optimize

In the implementation of EGM, we replace V_prime with a small positive number
if it is negative.

Consider speeding up EGM by writing a separate function for the Vp_cont?
"""
...
class DT_IFP(object):
    def __init__(self,rho=0.05,r=0.02,gamma=1,bnd=[[0,100],[-0.5,0.5]],
    ybar=1, mu=-np.log(0.95), sigma=np.sqrt(-2*np.log(0.95))*0.2, N=[80,20],
    N_c=2000, tol=10**-6, maxiter=20, maxiter_PFI=25, dt=0.01, NA=100,
    show_method = 1, show_iter = 1, show_final = 1):
        ...
        self.cgrid = np.linspace(10**(-4), 1.5*self.c0[-1,-1], self.N_c)
        self.b_prime0 = (1 + self.dt*self.r)*(self.xx[0] + self.dt*(self.ybar*np.exp(self.xx[1]) - self.c0))
    ...
    def polupdate_BF(self,V,stat=1):
        ...
        for i in range(self.N[0]-1):
            for j in range(self.N[1]-1):
                ...
                cgrid_rest = np.linspace(max(clow,10**-6), chigh, self.N_c)
...
class CT_stat_IFP(object):
    def __init__(self,rho=1/0.95-1, r=0.03, gamma=2.,mu=-np.log(0.95),
    sigma=np.sqrt(-2*np.log(0.95))*0.2,bnd=[[0,50],[-0.6,0.6]],N=(200,10),
    tol=10**-4, maxiter=200, maxiter_PFI=25, show_method = 1,
    show_iter = 1, show_final = 1, dt = 10**-4):
        self.rho, self.gamma = rho, gamma
        self.r, self.mu, self.sigma = r, mu, sigma
        self.tol, self.maxiter, self.maxiter_PFI = tol, maxiter, maxiter_PFI
...
class CT_nonstat_IFP(object):
    ...
    tol=10**-6,maxiter=200,maxiter_PFI=25,mono_tol = 10**(-6),
    show_method = 1, show_iter = 1, show_final = 1, dt = 10**(-4)):
        self.rho, self.gamma = rho, gamma
        self.r, self.mu, self.sigma = r, mu, sigma
    ...
    def p_func(self,ind,c):
        ...
        p_func[(1,0,0)] = d[0]*self.Delta[0]*np.maximum(self.r*x[0]+np.exp(x[1])-c,0)
        p_func[(-1,0,0)] = d[0]*self.Delta[0]*np.maximum(-(self.r*x[0]+np.exp(x[1])-c),0)
