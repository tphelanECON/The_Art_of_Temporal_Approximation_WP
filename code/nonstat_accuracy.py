"""
Comparison of brute force method with continuous-time method. Comparing both
across the methods and with their respective true values.
...
I don't think there should be any naive PFI here. Only put that in the naive_seq_accuracy
case. It will have to appear in the time script though, at least at some stage.
"""
...
def accuracy_data(N_set,DT_dt,CT_dt):
    ...
    Z['True'] = classes.CT_nonstat_IFP(rho=rho,r=r,gamma=gamma,mu=mu,sigma=sigma,
    ...
    DT['True'] = X['True'].nonstat_solve('BF')
    CT['True'] = Z['True'].solve_seq_imp()
    ...
    for N in N_set:
        ...
        tol=tol,show_method=show_method,show_iter=show_iter,show_final=show_final,dt=CT_dt)
        DT[N] = X[N].nonstat_solve('BF')
        CT[N] = Z[N].solve_seq_imp()
        def compare(f1,f2):
...
"""
DT, CT = accuracy_data(N_set,DT_dt,CT_dt)
"""
#diff = CT['True_seq'][1]-CT['True'][1]
...
DT_dt = 10**0
CT_dt = 10**-2
accuracy_tables(parameters.N_set, DT_dt, CT_dt)
